...
def create_pdf(url, args):
    ...
    """
    if url is None:
        logger.error("URL failed pre-tests. Exiting...")
        sys.exit(-1)
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            logger.warning("Cannot reach networkidle but will continue")
        ...
        logger.info("Saved to %s", temp_pdf_path)
        context.close()
        browser.close()
...
def download_pdf(url, args):
    ...
    """
    logger.info("Downloading PDF from %r", url)
    try:
    ...
    logger.info("Downloaded PDF to %r", temp_pdf_path)
    return temp_pdf_path
...
def url_arg_handler(url, args):
    ...
    if parsed_url.scheme == "file":
        ...
        if not parsed_path.is_file():
            logger.error("File %r not found", url)
            return None
        logger.info("Local file path %r is valid", parsed_path)
        # Return plain filesystem path for downstream libs
        return str(parsed_path)
    # Plain local path (no scheme/netloc)
    if parsed_url.scheme == "" and parsed_url.netloc == "":
        logger.info("Interpreting %r as a local file path", url)
    ...
    if url.endswith((".pdf", ".PDF")):
        logger.info("Interpreting %r as a PDF URL", url)
        # Download and return the local file path
        downloaded = download_pdf(url, args)
        if downloaded is None:
            logger.error("Failed to download PDF from %r", url)
            return None
        logger.info("PDF downloaded successfully from %r", url)
        return downloaded
    else:
        logger.info("Interpreting %r as a website URL", url)
        exported = create_pdf(url, args)
        if exported is None:
            logger.error("Failed to create PDF from website %r", url)
            return None
        logger.info("PDF created successfully from website %r", url)
        return exported
...
def main(url, output):
    ...
    """
    logger.info("Starting pdf_parser for %s -> %s", url, output)
    args = argparse.Namespace(url=url, output=output)
    pdf_path = url_arg_handler(args.url, args)

    if pdf_path is None:
        raise FileNotFoundError(f"Unable to produce a PDF from input {url}")

    # Convert the PDF to Markdown and then to HTML
    try:
        pdf_stat = Path(pdf_path).stat()
        logger.info(
            "Converting PDF to Markdown via pymupdf4llm (%s, size=%d bytes)",
            pdf_path,
            pdf_stat.st_size,
        )
    except OSError:
        logger.info("Converting PDF to Markdown via pymupdf4llm (%s)", pdf_path)

    # Extra debug logging around the conversion so we can see if it hangs
    try:
        logger.debug("Opening PDF with pymupdf4llm.to_markdown")
        md_text = pymupdf4llm.to_markdown(pdf_path, show_progress=True)
        logger.info("Finished PDF -> Markdown conversion (%s)", pdf_path)
    except Exception:
        logger.exception("Exception during PDF -> Markdown conversion for %s", pdf_path)
        raise
    ...
    logger.info("Writing HTML to %s", output_path)
    with open(output_path, "w", encoding="utf-8") as output_file:
        output_file.write(html)
    logger.info("pdf_parser completed for %s", output_path)
